...
{
    public class DishInteractor
    {
        private IRepository<Dish> repos;
        private IUnitWork unitWork;

        public DishInteractor(IRepository<Dish> repos, IUnitWork unitWork)
        {
            this.repos = repos;
            this.unitWork = unitWork;
        }
        public async Task<Response> Create(DishDto newDish)
        {
            var response = new Response<OrderDto>();
            try
            {
                await repos.CreateAsync(newDish.ToEntity());
        ...
        public async Task<Response<DishDto>> GetById(int id)
        {
            try
            {
        ...
        public async Task<Response<IEnumerable<DishDto>>> GetAll()
        {
            try
            {
                var list = await repos.GetAllAsync();
                if (list == null)
                    return new Response<IEnumerable<DishDto>>()
                    {
                        IsSuccess = true,
                        Value = null
                    };
        ...
        public async Task<Response> Update(DishDto updatedDish)
        {
            try
            {
                await repos.UpdateAsync(updatedDish.ToEntity());
                await unitWork.Commit();
                return new Response()
                {
                    IsSuccess = true
